...
create table USER_ORDER_INFO(
  ...
  USER_ORDER_CODE VARCHAR(20),  --用户订单编号 生成给用户看的标志 格式：用户ID(格式化四位)+商家ID(格式化四位)+序号五位（自增）
  ...
  USER_ORDER_STATE VARCHAR(5),  --用户订单状态 01 完成订单 02 待商家接单 03 商家已接单 04 烹饪中 05 配送中 99 异常
  ...
  USER_ORDER_ORDERID NUMBER(10)  --用户订单信息表关联订单信息
);
...
create table ORDER_INFO(
  ...
  ORDER_DISHED_ID VARCHAR(10),  -- 关联菜肴信息表
